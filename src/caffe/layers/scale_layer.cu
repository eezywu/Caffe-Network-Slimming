...
/****** add l1 regularization for gamma ******/
template <typename Dtype>
__global__ void scale_punish_func(const int n, const Dtype* scale, Dtype* scale_diff, Dtype lambda) {
  CUDA_KERNEL_LOOP(index, n) {
    if (scale[index] > 0.000000001){
      scale_diff[index] += lambda;
    }
    else if (scale[index] < -0.000000001){
      scale_diff[index] -= lambda;
    }
  }
}
...
// propagate_down： 梯度开关，即是否反向传播，size与输入的Blob的size相同
template <typename Dtype>
void ScaleLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if ((!scale_param && propagate_down[1]) ||
    ...
    caffe_gpu_mul(top[0]->count(), top_diff, bottom_data, product);// gamma 求导公式中的element-wise相乘，未求和
    if (!is_eltwise) {
      // Here
      ...
      if (outer_dim_ != 1) {
        ...
        if (scale_dim_ == 1) {
          ...
          caffe_gpu_gemv(CblasTrans, outer_dim_, scale_dim_,
                         ...
                         scale_diff);
          /****** add l1 regularization for gamma ******/               
          const Dtype* scale_params = scale->gpu_data(); 
          scale_punish_func<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
              <<<CAFFE_GET_BLOCKS(scale_dim_), CAFFE_CUDA_NUM_THREADS>>>(
                scale_dim_, scale_params, scale_diff, l1_lambda_);
        }
      }
    }
  }
  // const int, const float *, int, int, const float *
  // const int n, const Dtype* scale, 
  // const int scale_dim, const int inner_dim, Dtype* out
  // 求对统计x的导数，以便传到BN层
  if (propagate_down[0]) {
    const int count = top[0]->count();
